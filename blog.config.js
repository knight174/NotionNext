...
const BLOG = {
  ...
  REVALIDATION_TOKEN: process.env.REVALIDATION_TOKEN || '', // On-Demand Revalidation Token，可通过 POST /api/revalidate 立即刷新页面缓存。
  ...
  AUTHOR: process.env.NEXT_PUBLIC_AUTHOR || '菠萝的蜜', // 您的昵称 例如 tangly1024
  BIO: process.env.NEXT_PUBLIC_BIO || '专业、实用、人文', // 作者简介
  LINK: process.env.NEXT_PUBLIC_LINK || 'https://blog.miaoosi.com', // 网站地址
  KEYWORDS:
    process.env.NEXT_PUBLIC_KEYWORD ||
    '全栈JS工程师,前端开发者,后端开发者,Web技术,教程,资讯,效率工具,专业、实用、人文的前端技术网站,技术爱好者,行业精英', // 网站关键词 英文逗号隔开
  ...
  // 覆盖 conf/image.config.js：本地博客的图片压缩与随机封面偏好
  ...
  RANDOM_IMAGE_URL:
    process.env.NEXT_PUBLIC_RANDOM_IMAGE_URL ||
    'https://bingw.jasonzeng.dev/?index=random', // 随机图片API,如果未配置下面的关键字，主页封面，头像，文章封面图都会被替换为随机图片
  ...
  GREETING_WORDS:
    ...
    '不要急于决断生死、判定正义，即便智者也不能洞悉万物的结局。—— 指环王1：护戒使者',
